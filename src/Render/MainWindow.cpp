...
namespace vrv
{
   ...
   public:
      ...
      void registerWindow(HWND hwnd, MainWindow* window)
      {
      ...
      MainWindow* getMainWindow(HWND hwnd)
      {
         if (myWindowHandles.find(hwnd) != myWindowHandles.end())
         {
   ...
   protected:
      typedef std::map<  HWND, MainWindow* >  WindowHandles;
      WindowHandles myWindowHandles;
   };

   LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
   {
      MainWindow* window = Win32WindowingSystem::instance()->getMainWindow(hwnd);
      if (window)
      {
   ...
   MainWindow::MainWindow(Viewer* viewer)
      : myWindowHandle(0)
      , myDeviceContext(0)
      , myOpenGLContext(0)
      , myWindowClassName("MainWindowClass")
      , myWindowName("MainWindow")
      , myWidth(800)
      , myHeight(600)
      , myViewer(viewer)
   {
   ...
   LRESULT MainWindow::handleNativeWindowingEvent(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
   {
      switch (uMsg)
      {
      case WM_SIZE:
   ...
   bool MainWindow::createWindow()
   {
      myWindowHandle = CreateWindowEx(WS_EX_CLIENTEDGE,
         ...
         myWindowName.c_str(),
         WS_OVERLAPPEDWINDOW,
         CW_USEDEFAULT, CW_USEDEFAULT, myWidth, myHeight, NULL, NULL, GetModuleHandle(NULL), NULL);
      if (!myWindowHandle)
      {
   ...
   void MainWindow::onResize(UINT flag, int width, int height)
   {
      myWidth = width;
      myHeight = height;
