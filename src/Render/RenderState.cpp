...
namespace vrv
{
	...
	void DepthTest::apply(bool forceUpdate)
	{
		...
		if (myEnabled)
		{
         ...
         glDepthFunc(myDepthTestFunction_GL);
		}
		else
		{
	...
	void StencilTest::apply(bool forceUpdate)
	{
		...
		if (myEnabled)
		{
         ...
         glStencilMask(myStencilWriteMask);
         glStencilFunc(myStencilFunction_GL, myStencilRef, myStencilMask);
         glStencilOp(myStencilOperation_sfail_GL,
