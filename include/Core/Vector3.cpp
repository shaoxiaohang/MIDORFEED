...
namespace vrv
{
   ...
   template<typename T>
   Vector3<T>::Vector3<T>(Vector2<T> vec)
	{
      data[0] = vec[0];
      data[1] = vec[1];
      data[2] = 0;
   ...
   template<typename T>
   Vector3<T> Vector3<T>::operator /(T mag) const
	{
   ...
   template<typename T>
   Vector3<T> Vector3<T>::operator *(T mag)
	{
      return Vector3<T>(data[0] * mag, data[1] * mag, data[2] * mag);
	}

   template<typename T>
   float Vector3<T>::operator[] (unsigned int index) const
   {
      return data[index];
   }
